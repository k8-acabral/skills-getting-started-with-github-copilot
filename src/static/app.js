document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear loading message and rebuild dropdown options
          ...
          ? `<ul class="participants-list">${details.participants
              .map((email) => `
                <li class="participant-item">
                  <span>${email}</span>
                  <button type="button" class="participant-remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove participant">&times;</button>
                </li>`)
              .join("")}</ul>`
